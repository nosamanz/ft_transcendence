...
app.get('/', (req, res) => {
    res.sendFile('/home/osarihan/Desktop/ft_transcendence-game/game/game/src/menu.html');
});
let username = "default";
let room_code = "";
app.get('/public/:username/', (req, res) => {
    res.sendFile('/home/osarihan/Desktop/ft_transcendence-game/game/game/src/game.html');
...
const lobby = new lobby_1.default();
const room_manager = new room_manager_1.default(game_io);
game_io.on('connection', (socket) => {
    ...
    if (lobby.get_num_player() % 2 == 0 && lobby.get_num_player() > 0) {
        const player1 = lobby.public_queue.shift();
        const player2 = lobby.public_queue.shift();
        room_manager.create_room(player1, player2);
    }
    if (lobby.get_num_private_players(room_code) == 2) {
        const player1 = lobby.private_players[room_code].shift();
        const player2 = lobby.private_players[room_code].shift();
        room_manager.create_room(player1, player2);
    }
    ...
    socket.on('keydown', (keycode) => {
        if (room_manager.num_rooms > 0) {
            const user = room_manager.find_user(socket.id);
            if (user != null) {
                if (keycode != 32) {
